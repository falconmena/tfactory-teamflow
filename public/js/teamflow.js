...
const teamFlowDropzoneInit = () => {
    ...
    const dropzone = new window.Dropzone(myDropzone, userOptions);

    dropzone.on(Events.ADDED_FILE, () => {
        if (myDropzone.querySelector(Selector.DZ_PREVIEW_COVER)) {
            myDropzone.querySelector(
                Selector.DZ_PREVIEW_COVER
            ).classList.remove(ClassName.DZ_FILE_COMPLETE);
        }
        myDropzone.classList.add(ClassName.DZ_FILE_PROCESSING);
    });
    dropzone.on(Events.REMOVED_FILE, () => {
        ...
        myDropzone.classList.add(ClassName.DZ_FILE_COMPLETE);
    });
